...
**Micro-lot visuel (meme branche, suite relecture Xavier).** Carte d'index encadree avec vignette (image + titre + date + temps de lecture + description, tout cliquable). Image de perroquet choisie dans la reserve `_image-sources/` (le "thank you" multilingue du 14/07, langues alignees sur les cibles, aucun wording de gratuite interdit, ecarte les visuels portant "FOREVER"). Deux derives web optimises via PIL : `img/blog/how-i-built-an-ielts-site-with-ai.webp` (hero + vignette, 900px, 70 Ko) et `img/blog/how-i-built-an-ielts-site-with-ai-og.jpg` (social 1200x630, 130 Ko). Image integree en vignette de carte, en tete d'article (`.article-hero`), et dans les metas (og:image, twitter:image, JSON-LD image). Gabarit `_template-article.html` dote de l'emplacement image standard a jetons (`{{SLUG}}.webp`, `{{SLUG}}-og.jpg`, `{{IMAGE_ALT}}`). Tests : `check.js` **108/0** (7 assertions image neuves), `e2e-blog` **40/0** (vignette, hero, og:image, decodage reel des images et poids web). Captures 1280 et 390 regenerees. Grep wording final : 0 forever/always free/for now. 0 tiret cadratin.
